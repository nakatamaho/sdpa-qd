...
Vector::Vector(int nDim, qd_real value)
{
  ele  = NULL;
...
void Vector::initialize(int nDim,qd_real value)
{
...
void Vector::initialize(qd_real value)
{
  if (nDim<=0) {
    rError("Vector:: nDim is nonpositive");
  }
...
void Vector::display(FILE* fpout,qd_real scalar)
{
  if (fpout == NULL) {
    return;
  }
  fprintf(fpout,"{");
  for (int j=0; j<nDim-1; ++j) {
    qd_real mtmp=ele[j]*scalar;
    fprintf(fpout,P_FORMAT",",mtmp.x[0]);
  }
  if (nDim>0) {
    qd_real mtmp=ele[nDim-1]*scalar;
    fprintf(fpout,P_FORMAT"}\n",mtmp.x[0]);
  } else {
    fprintf(fpout,"  }\n");
  }
...
void BlockVector::initialize(qd_real value)
{
	   ...
	   int NonZeroNumber)
{
  ...
  case SPARSE:
    ...
    if (NonZeroNumber > 0) {
      ...
      rNewCheck();
      sp_ele       = new qd_real[NonZeroNumber];
  ...
  case DENSE:
    ...
    rNewCheck();
    de_ele = new qd_real[NonZeroNumber];
    if (de_ele==NULL) {
      rError("SparseMatrix:: memory exhausted");
    }
    length = nRow*nCol;
...
void SparseMatrix::setIdentity(qd_real scalar)
{
  if (nRow != nCol) {
...
bool SparseMatrix::sortSparseIndex(int& i, int& j)
{
  ...
  i = -1;
  j = -1;
  const qd_real tolerance = 1.0e-8;
  switch(type) {
  case SPARSE:
...
bool DenseMatrix::copyFrom(SparseMatrix& other)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index = 0; index<other.NonZeroCount; ++index) {
      ...
      qd_real value = other.sp_ele[index];
      de_ele[i+nCol*j] = de_ele[j+nCol*i] = value;
    }
    break;
...
void DenseMatrix::setIdentity(qd_real scalar)
{
  if (nRow != nCol) {
    rError("SparseMatrix:: Identity matrix must be square matrix");
  }
				   ...
				   bool* LP_NonZeroNumber)
{
  ...
  if (LP_sp_nBlock > 0){
	...
	LP_sp_block = NULL;
	rNewCheck();
	LP_sp_block = new qd_real[LP_sp_nBlock];
	if (LP_sp_block==NULL) {
	  rError("SparseLinearSpace:: memory exhausted");
	}
  }
                                   ...
                                   int* LP_sp_index)
{
  ...
  if (LP_sp_nBlock > 0){
	...
	rNewCheck();
	this->LP_sp_block = new qd_real[LP_sp_nBlock];
...
void SparseLinearSpace::setElement_SDP(int block, int i, int j, qd_real ele)
{
  int k;
...
void SparseLinearSpace::setElement_SOCP(int block, int i, int j, qd_real ele)
{
  rError("DenseLinearSpace:: current version does not support SOCP");
}

void SparseLinearSpace::setElement_LP(int block, qd_real ele)
{
  int k;
...
void SparseLinearSpace::setIdentity(qd_real scalar)
{
				  ...
				  int LP_nBlock)
{
  ...
  if ((LP_nBlock > 0) && (LP_block==NULL)) {
    rNewCheck();
    LP_block = new qd_real[LP_nBlock];
    if (LP_block==NULL) {
      rError("DenseLinearSpace:: memory exhausted");
    }
  }
...
void DenseLinearSpace::setElement_SDP(int block, int i, int j, qd_real ele)
{
...
void DenseLinearSpace::setElement_SOCP(int block, int i, int j, qd_real ele)
{
  rError("DenseLinearSpace:: current version does not support SOCP");
}

void DenseLinearSpace::setElement_LP(int block, qd_real ele)
{
  // check range
  if (block >= LP_nBlock){
    rError("out of range in input data");
...
void DenseLinearSpace::setIdentity(qd_real scalar)
{
  // for SDP
  if (SDP_nBlock>0 && SDP_block) {
    for (int l=0; l<SDP_nBlock; ++l) {
...
//print vector as matlab compat format.
void printveci (int N, qd_real *A, int inc)
{
  int ix=0;
  printf(" [ ");
  for (int i=0; i<N-1; i++){
...
void printsymmmat (int N, qd_real *A, int LDA)
{
  qd_real mtmp;
  printf("[ ");
  for (int i=0; i<N; i++){
...
void printmat3 (int N, qd_real *p, qd_real *q)
{
  qd_real mtmp;
  qd_real zero;
  zero = 0.0;
