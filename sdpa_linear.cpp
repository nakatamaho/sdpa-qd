...
qd_real Lal::getMinEigen(DenseMatrix& lMat,
		       DenseMatrix& xMat,
		       DenseMatrix& Q,
		       ...
		       Vector& workVec)
{
  qd_real alpha,beta,value;
  qd_real min = 1.0e+51, min_old = 1.0e+52, min_min= 1.0e+50;
  qd_real error = 1.0e+10;
  ...
  q.setZero();
  r.initialize(MONE);
  beta = sqrt((qd_real)nDim);  // norm of "r"

  // nakata 2004/12/12
  while (k<nDim && k<sqrt((qd_real)nDim)+10
	 && beta > 1.0e-16
    ...
    if ( kk>=sqrt((qd_real)k) || k==nDim-1 || k>sqrt((qd_real)nDim+9) ) {
      kk = 0;
...
qd_real Lal::getMinEigenValue(DenseMatrix& aMat,
			     Vector& eigenVec,
			     Vector& workVec)
{
  // aMat is rewritten.
...
bool Lal::getInnerProduct(qd_real& ret, Vector& aVec, Vector& bVec)
{
  int N = aVec.nDim;
  if (N != bVec.nDim) {
...
bool Lal::getInnerProduct(qd_real& ret,
			  ...
			  SparseMatrix& aMat, DenseMatrix& bMat)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index=0; index<aMat.NonZeroCount; ++index) {
      ...
      qd_real value = aMat.sp_ele      [index];
      // rMessage("i=" << i << "  j=" << j);
      if (i==j) {
    ...
    for (int index=amari,counter = 0;
	 counter < shou ; ++counter, index+=4) {
      ...
      qd_real value1 = aMat.sp_ele      [index];
      qd_real ret1;
      // rMessage("i=" << i << "  j=" << j);
      if (i1==j1) {
	...
	ret1= value1*(bMat.de_ele[i1+bMat.nRow*j1]
      ...
      qd_real value2 = aMat.sp_ele      [index+1];
      qd_real ret2;
      // rMessage("i=" << i << "  j=" << j);
      if (i2==j2) {
	...
	ret2= value2*(bMat.de_ele[i2+bMat.nRow*j2]
      ...
      qd_real value3 = aMat.sp_ele      [index+2];
      qd_real ret3;
      // rMessage("i=" << i << "  j=" << j);
      if (i3==j3) {
	...
	ret3= value3*(bMat.de_ele[i3+bMat.nRow*j3]
      ...
      qd_real value4 = aMat.sp_ele      [index+3];
      qd_real ret4;
      // rMessage("i=" << i << "  j=" << j);
      if (i4==j4) {
...
bool Lal::choleskyFactorWithAdjust(DenseMatrix& aMat)
{
  ...
  return _SUCCESS;
#if 0
  qd_real ZERO_DETECT = 1.0e-3;
  qd_real NONZERO = 1.0e-7;
  // no idea version
  ...
  while (start<aMat.nRow) {
    ...
    qd_real wrong = aMat.de_ele[start+start*aMat.nRow];
	...
	qd_real& migi  = aMat.de_ele[start+(start+j)*aMat.nRow];
	qd_real& shita = aMat.de_ele[(start+j)+start*aMat.nRow];
	qd_real& mishi = aMat.de_ele[(start+j)+(start+j)*aMat.nRow];
		   ...
		   qd_real* scalar)
{
  ...
  case SparseMatrix::SPARSE:
	...
	int        i = aMat.row_index   [index];
	int        j = aMat.column_index[index];
	qd_real value = aMat.sp_ele      [index] * (*scalar);
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
	    retMat.de_ele[i+retMat.nRow*t] +=
		   ...
		   qd_real* scalar)
{
  ...
  case SparseMatrix::SPARSE:
	...
	qd_real value = bMat.sp_ele      [index];
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
	...
	int        i = bMat.row_index   [index];
	int        j = bMat.column_index[index];
	qd_real value = bMat.sp_ele      [index] * (*scalar);
	if (i!=j) {
	  #if MULTIPLY_NON_ATLAS
	  for (int t=0; t<bMat.nCol; ++t) {
...
bool Lal::multiply(DenseMatrix& retMat,
		   DenseMatrix& aMat, qd_real* scalar)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index=amari,counter=0;
	 counter<shou; ++counter,index+=4) {
      ...
      qd_real value1 = bMat.sp_ele      [index] * (*scalar);
      if (i1!=j1) {
	retMat.de_ele[i1+retMat.nCol*j1] += value1;
	retMat.de_ele[j1+retMat.nCol*i1] += value1;
      } else {
	retMat.de_ele[i1+retMat.nCol*i1] += value1;
      ...
      qd_real value2 = bMat.sp_ele      [index+1] * (*scalar);
      if (i2!=j2) {
	retMat.de_ele[i2+retMat.nCol*j2] += value2;
	retMat.de_ele[j2+retMat.nCol*i2] += value2;
      } else {
	retMat.de_ele[i2+retMat.nCol*i2] += value2;
      ...
      qd_real value3 = bMat.sp_ele      [index+2] * (*scalar);
      if (i3!=j3) {
	retMat.de_ele[i3+retMat.nCol*j3] += value3;
	retMat.de_ele[j3+retMat.nCol*i3] += value3;
      } else {
	retMat.de_ele[i3+retMat.nCol*i3] += value3;
      ...
      qd_real value4 = bMat.sp_ele      [index+3] * (*scalar);
      if (i4!=j4) {
	retMat.de_ele[i4+retMat.nCol*j4] += value4;
	retMat.de_ele[j4+retMat.nCol*i4] += value4;
      } else {
...
bool Lal::plus(BlockVector& retVec,
	       BlockVector& aVec,
	       BlockVector& bVec, qd_real* scalar)
{
...
bool Lal::let(DenseMatrix& retMat, const char eq,
	      ...
	      DenseMatrix& bMat, qd_real* scalar)
{
  qd_real minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(DenseMatrix& retMat, const char eq,
	      DenseMatrix& aMat, const char op,
	      SparseMatrix& bMat, qd_real* scalar)
{
  qd_real minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(qd_real& ret, const char eq,
	      Vector& aVec, const char op,
	      Vector& bVec)
{
  switch (op) {
...
bool Lal::let(DenseLinearSpace& retMat, const char eq,
	      DenseLinearSpace& aMat, const char op,
	      DenseLinearSpace& bMat, qd_real* scalar)
{
  qd_real minus_scalar;
  switch (op) {
  case '+':
...
bool Lal::let(DenseLinearSpace& retMat, const char eq,
	      DenseLinearSpace& aMat, const char op,
	      SparseLinearSpace& bMat, qd_real* scalar)
{
  qd_real minus_scalar;
  switch (op) {
  case '+':
