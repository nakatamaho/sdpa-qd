...
Solutions::Solutions(int m,
		     ...
		     int LP_nBlock, qd_real lambda, ComputeTime& com)
{
...
qd_real Residuals::computeMaxNorm(Vector& primalVec)
{
  qd_real ret = 0.0;
  #if 1
  for (int k=0; k<primalVec.nDim; ++k) {
    qd_real tmp = abs(primalVec.ele[k]);
    if (tmp > ret) {
      ret = tmp;
    }
  }
...
qd_real Residuals::computeMaxNorm(DenseLinearSpace& dualMat)
{
  ...
  for (int l=0; l<SDP_nBlock; ++l) {
    qd_real* target = dualMat.SDP_block[l].de_ele;
