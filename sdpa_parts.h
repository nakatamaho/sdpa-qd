...
public:
  ...
  qd_real dual;
  StepLength();
  StepLength(qd_real alphaP, qd_real alphaD, int nBlock,
	      int* blockStruct);
  ~StepLength();
  void initialize(qd_real alphaP, qd_real alphaD);
  void terminate();
  
  static qd_real minBlockVector(BlockVector& aVec);
...
public:
  qd_real value;
  DirectionParameter(qd_real betaStar=0.0);
  ~DirectionParameter();
  void initialize(qd_real betaStar=0.0);
...
public:
  qd_real initial;
  qd_real current;
  AverageComplementarity(qd_real lambdaStar = 0.0);
  ~AverageComplementarity();
  void initialize(qd_real lambdaStar = 0.0);
